...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <p class="text-gray-600 text-sm">Painel de gestão do inventário de veículos</p>
            </div>
    ...
    <div id="carModal" class="fixed inset-0 z-50 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 {{ $errors->any() ? '' : 'hidden' }}">
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <!-- Formulário com suporte para ficheiros (enctype) -->
            <form id="carForm" action="{{ route('cars.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col flex-1 overflow-hidden">
                @csrf
                ...
                <div class="p-6 overflow-y-auto space-y-4 flex-1">
                    ...
                    <div>
                        ...
                        <span class="text-xs text-gray-500">Formatos aceites: JPG, PNG, WEBP (Máx. 2MB)</span>
                    </div>

                    <!-- Categoria -->
                    <div>
                        <label for="category_id" class="block text-sm font-medium text-gray-700 mb-1">Categoria</label>
                        <select name="category_id" id="category_id" class="w-full border-gray-300 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2.5 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="marca" id="marca" value="{{ old('marca') }}" placeholder="Ex: Toyota" class="w-full border-gray-300 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2.5 text-sm" required>
                    </div>

                    <!-- Modelo -->
                    <div>
                        <label for="modelo" class="block text-sm font-medium text-gray-700 mb-1">Modelo</label>
                        <input type="text" name="modelo" id="modelo" value="{{ old('modelo') }}" placeholder="Ex: Land Cruiser" class="w-full border-gray-300 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2.5 text-sm" required>
                    </div>

                    <!-- Cor -->
                    <div>
                        <label for="cor" class="block text-sm font-medium text-gray-700 mb-1">Cor</label>
                        <select name="cor" id="cor" class="w-full border-gray-300 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2.5 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="ano" id="ano" value="{{ old('ano') }}" placeholder="Ex: 2024" class="w-full border-gray-300 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2.5 text-sm" required>
                    </div>

                    <!-- Placa -->
                    <div>
                        <label for="placa" class="block text-sm font-medium text-gray-700 mb-1">Placa / Matrícula</label>
                        <input type="text" name="placa" id="placa" value="{{ old('placa') }}" placeholder="Ex: LDA-28-62-RP" class="w-full border-gray-300 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2.5 text-sm" required>
                    </div>

                    <!-- Preço -->
                    <div>
                        <label for="preco" class="block text-sm font-medium text-gray-700 mb-1">Preço (KZ)</label>
                        <input type="number" step="0.01" name="preco" id="preco" value="{{ old('preco') }}" placeholder="Ex: 25000000" class="w-full border-gray-300 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2.5 text-sm" required>
                    </div>
    ...
    <!-- JavaScript para Controlo de Modais -->
    <script>
